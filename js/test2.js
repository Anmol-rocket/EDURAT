// Base URL constant
const baseUrl = "http://192.168.137.1:8081";
...
function redirectToGradePage(grade) {
  ...
  if (grade >= 0 && grade <= 5) {
    ...
    // For parent logins, call API to get child list
    fetch(`${baseUrl}/getChild/${encodeURIComponent(localStorage.getItem("parentUsername"))}`)
      .then(response => response.json())
      .then(childList => {
        if (childList && Array.isArray(childList) && childList.length > 0) {
          // Store the first child username as loginUsername
...
// Updated handleLogin function with new API call for parents
async function handleLogin(event) {
  ...
  try {
    ...
    if (grade === -1) {
      // For parent, call getChild API to fetch child list
      fetch(`${baseUrl}/getChild/${encodeURIComponent(username)}`)
        .then(response => response.json())
        .then(childList => {
          if (childList && Array.isArray(childList) && childList.length > 0) {
            // Store the first element as loginUsername
            localStorage.setItem("loginUsername", childList[0]);
          }
